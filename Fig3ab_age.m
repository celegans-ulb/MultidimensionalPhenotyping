...
for ii = 1:numel(nbFeat)
    ...
    for jj = 1:numel(myFeatures)
        ...
        % get predMSE
        myPredMSE(jj) = sqrt(nanmean((testAges - predValues).^2));
...
for ii = 1:numel(inputFeatNames)
    ...
    % get predMSE
    predMSE2(ii) = sqrt(nanmean((testAges - predValues).^2));
...
% initialise variable to store MSE and features
...
for ii = 1:100
    ...
    % train svm
    myModel = fitrsvm(trainData(:,myFeatCumInd),trainAges);
    
    % predict
    predValues = predict(myModel,testData(:,myFeatCumInd));
    
    % get predMSE
    predMSEind(ii) = sqrt(nanmean((testAges - predValues).^2));
...
for ii = 1:100
    ...
    % get predMSE
    predMSEcum(ii) = sqrt(nanmean((testAges - predValues).^2));
...
title('Age prediction','Fontsize',18);
...
hold on;
plot(testAges,testAges,'Linewidth',2);
hold off;
...
% train SVM
myModel = fitrsvm(trainData,trainAges);

% predict
predValues = predict(myModel,testData);

% get RMSE
predRMSEall = sqrt(nanmean((testAges - predValues).^2));

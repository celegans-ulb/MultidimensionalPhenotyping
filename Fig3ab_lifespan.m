...
% trainLifespans = inputLifespans(indexTrain);
...
for ii = 1:numel(nbFeat)
    ...
    for jj = 1:numel(myFeatures)
        ...
        % get lifespan
        predLifespans = testAges + predValues;
        
        % get predMSE
        myPredMSE(jj) = sqrt(nanmean((testLifespans - predLifespans).^2));
...
for ii = 1:numel(inputFeatNames)
    ...
    % get predMSE
    predMSE2(ii) = sqrt(nanmean((testLifespans - predLifespans).^2));
...
for ii = 1:100
    ...
    % get predMSE
    predMSEind(ii) = sqrt(nanmean((testLifespans - predLifespans).^2));
...
for ii = 1:100
    ...
    % get predMSE
    predMSEcum(ii) = sqrt(nanmean((testLifespans - predLifespans).^2));
...
title('Lifespan prediction','Fontsize',18);
...
% get RMSE
predRMSE = sqrt(nanmean((testLifespans - predLifespans).^2));
        
% plot
figure;
scatter(testLifespans,predLifespans);
hold on;
plot(testLifespans,testLifespans,'Linewidth',2);
hold off;
xlabel('Real age (days)','Fontsize',16)
ylabel('Predicted values (days)','Fontsize',16)


%% Get R2

[fitobject,gof] = fit(testAges,predValues,'poly1');
...
% get RMSE
predLifespans = testAges + predValues;
predRMSEall = sqrt(nanmean((testLifespans - predLifespans).^2));
